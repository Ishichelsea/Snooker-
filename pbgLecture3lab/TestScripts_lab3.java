...
public class TestScripts_lab3 {
	...
	@Test
    public void testSideSpinApplication() {
        double ballRadius = 0.1; 
        double ballMass = 1; 
        BasicParticle cueBall = new BasicParticle(5, 5, 0, 0, ballRadius, true, Color.WHITE, ballMass);
        
        // Applying side spin to the cue ball
        Vect2D sideSpin = new Vect2D(0.5, -0.5);
        cueBall.setSideSpin(sideSpin);
        
        // Call update to simulate the physics update
        cueBall.update(0, 0.1); // Using a gravity of 0 and a small delta time

        // Check if the cue ball's velocity is updated correctly
        Vect2D expectedVelocityAfterSpin = sideSpin; 
        assertEquals("Side spin should be applied to velocity", expectedVelocityAfterSpin, cueBall.getVel());
    }	
}
